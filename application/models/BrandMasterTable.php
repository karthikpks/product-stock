...
class BrandMasterTable extends CI_Model{
	...
	public function updateBrand() {
		$this->brandMasterId = strtolower($this->security->xss_clean($this->input->post('brandMasterId')));
		$this->brandMasterDesc = strtolower($this->security->xss_clean($this->input->post('brandMasterDesc')));
		...
		if($this->checkbrandExits($this->brandMasterDesc) && !empty($this->brandMasterDesc)) {
			$insertData = array(
					$this->brandColumn[1] => $this->brandMasterDesc,
					$this->brandColumn[2] => $this->cm_id,
					$this->brandColumn[4] => date('Y-m-d'),
					$this->brandColumn[5] => 1,
				);
			$this->db->where($this->brandColumn[0], $this->brandMasterId);
	...
	public function getAllBrandList() {
		...
		if(!empty($this->searchKey)) {
            $this->db->or_like(array($this->brandColumn[1] => $this->searchKey));
        }
